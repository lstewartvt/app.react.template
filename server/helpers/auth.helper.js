...
let auth_helper = module.exports = {
  check: passport.authenticate('jwt', {
    failureRedirect: config.app.nav.account.login,
    session: false
  }),
      ...
      return existingUser.isPasswordValid(password.trim())
        .then(function(isValid) {
          ...
          if (isValid) {
            ...
            return done(null, existingUser.transform());
          } else {
  ...
  verify: (request, response) => {
    ...
    if (!token) {
      return response.status(403).redirect(config.app.nav.restricted);
    }
  ...
  verifyResend: (request, response) => {
    ...
    return User.getUserById(request.params.user_id).then(user => {
      ...
      return Verify.getUserToken(user._id).then(function(verify) {
        ...
        if (!verify.success) {
          ...
          const config = {
            callback: () => {
              ...
              // return the user information as JSON
              return response.redirect(config.app.nav.account.loginResend.replace('{success}', true));
            },
            host: request.get('host'),
            protocol: request.protocol,
            user: user
        ...
        console.log(error);
        return response.redirect(config.app.nav.account.loginResend.replace('{success}', false));
      });
    })
  },
  verifySend: (user, token_url, callback) => {
    ...
    const message = {
      config: {
        ...
        to: user.email,
        expires: config.server.verify.expireText,
        url: token_url
      },
      template: 'verify'
    };
